...
- [为录像视频生成时间字幕](https://github.com/f-shake/TimeAss4Video)
...
| 文件筛选操作                                              | 文件可读性检测                                               |
| --------------------------------------------------------- | ------------------------------------------------------------ |
| ![10.文件筛选操作](imgs/文件目录工具/10.文件筛选操作.png) | ![11.文件可读性检测](imgs/文件目录工具/11.文件可读性检测.png) |

## 文本工具

| 文本智能生成（AI）                                  | 文本简单加密                                        | 文档智能搜索（AI）                                  |
| --------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------- |
| ![1.文本智能生成](imgs/文本工具/1.文本智能生成.png) | ![2.文本简单加密](imgs/文本工具/2.文本简单加密.png) | ![3.文档智能搜索](imgs/文本工具/3.文档智能搜索.png) |
...
| 照片标签生成                                            | 照片标签管理                                            | 照片标签搜索                                                |
| ------------------------------------------------------- | ------------------------------------------------------- | ----------------------------------------------------------- |
| ![1.照片标签生成](imgs/照片标签工具/1.照片标签生成.png) | ![2.照片标签管理](imgs/照片标签工具/2.照片标签管理.png) | ![3.根据关键词查找](imgs/照片标签工具/3.根据关键词查找.png) |

## 视频工具

| 视频时间戳字幕                                          |
| ------------------------------------------------------- |
| ![1.视频时间戳字幕](imgs/视频工具/1.视频时间戳字幕.png) |
...
| 文件可读性检测<br />`FileReadabilityScanner` | 检查文件是否可以完整读取，并提供二进制比特的分布 | 磁盘可能产生损坏，需要检查是否有文件不可读；或文件有一场的可能性，检查文件中比特分布是否均匀 |  |
...
| 文本简单加密<br />`TextEncryption`      | 使用替换式密码的方式加密文本，实现防君子不防小人的文本加密   | 保护一般敏感信息（而不是高敏感性信息），实现轻量级可逆文本加密 |        | [文本加密工具](https://github.com/f-shake/TextEncryption) |
...
[照片标签帮助](./helps/photoTagging.md)

| 工具名                                | 用途                                     | 调用AI |
| ------------------------------------- | ---------------------------------------- | ------ |
| 照片标签生成<br />`PhotoTagGenerator` | 通过AI，自动生成照片的关键词标签         | √      |
| 图像标签管理<br />`PhotoTagManager`   | 管理各图像的标签，进行新增、修改或和删除 |        |
| 照片标签搜索<br />`PhotoTagSearcher`  | 根据关键词，查找指定标签的图像           |        |
...
| 工具名                        | 用途                     | 期望解决的问题                                       | 原项目                                                       |
| ----------------------------- | ------------------------ | ---------------------------------------------------- | ------------------------------------------------------------ |
| 视频时间戳字幕<br />`TimeAss` | 为视频添加录制时间的字幕 | 对于一份拍摄的视频，希望能够在播放时了解拍摄时的时刻 | [为录像视频生成时间字幕](https://github.com/f-shake/TimeAss4Video) |
