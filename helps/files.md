...
文件可读性检测是一款专业的文件健康状态扫描工具，能够批量检测指定目录下所有文件的物理可读性，并统计文件中比特位的分布比例。该工具通过逐字节读取文件内容，分析比特位中0和1的数量，从而评估文件的存储健康状态，特别适用于长期存储介质的定期检查、数据迁移前的完整性验证以及异常文件的筛查。

## 功能特性

1. 批量文件检测：支持对指定目录下所有文件进行批量可读性检测，自动应用过滤规则
2. 可读性状态标识：明确标记每个文件是否可完整读取，并用颜色区分（绿色为可读，红色为不可读）
3. 比特位统计分析：统计每个文件中比特位0和1的数量，计算比特位比例，帮助识别异常文件
4. 智能预警机制：当文件中比特位为1的比例小于1%时，工具会发出警告，提示文件可能已损坏或为特殊格式
5. 进度实时反馈：检测过程显示详细进度，包括当前处理的文件序号和文件名，支持取消操作

## 配置说明

1. 目录设置：选择需要检测文件可读性的目标文件夹
2. 过滤规则：通过文件过滤规则（包括文件名、扩展名、大小、时间等条件）筛选需要检测的文件，避免不必要的文件被纳入检测

## 原理解析

该工具采用两阶段处理流程：初始化阶段遍历指定目录下的所有文件，应用过滤规则后生成待检测文件列表；执行阶段逐文件进行可读性检测。可读性检测通过打开文件并尝试完整读取其内容来实现，读取过程中统计每个字节的比特位，计算0和1的数量。如果文件读取过程中发生任何I/O异常，则该文件被标记为不可读。工具采用流式读取和固定大小缓冲区，避免内存溢出。检测结果以列表形式展示，包括文件的可读状态、比特位统计和比例，用户可根据结果快速定位问题文件。同时，工具提供比特位比例预警，当文件中比特位1的比例过低时（如小于1%），提示文件可能已损坏或为全0填充的特殊文件。
