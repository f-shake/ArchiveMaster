...
3. 压缩参数：
   ...
   - JPEG质量：设置压缩质量（界面可调范围 10-100）
   - 输出格式：指定压缩后的文件格式。`jpg`兼容性最好；若目标设备支持、且需要更小的体积，可选 `heic`、`avif`、`webp` 等。
     - `heic` 需要随包的原生库（Windows 版已内置）；原生库不可用时，该项不会出现在格式列表中，其它格式不受影响。
     - HEIC 的"质量"刻度与 JPEG 不可比，同样的数值观感差异较大，建议先用少量照片试看再批量处理。
     - HEIC 输出使用 `.heic` 扩展名，实测在手机相册上的兼容性优于 `.heif`。
     - 注意：HEIC 编码不支持"接近无损"的档位（高质量区间的体积可能反而大于 JPEG），追求最高画质时建议仍用 JPEG。
