    ...
    : AiTwoStepServiceBase<LineByLineProcessorConfig>(appConfig)
{
    ...
    public override async Task ExecuteAsync(CancellationToken ct = default)
    {
        ...
        await Task.Run(async () =>
        {
            ...
            foreach (var chunk in chunks)
            {
                ...
                NotifyProgress(chunkIndex * Config.MaxLineEachCall, Items.Count);
                chunkIndex++;
                NotifyCurrentMessage(chunkIndex, chunkCount);

                //循环每一轮投票，并发执行
                var tasks = new List<Task>();
                foreach (var voteIndex in voteIndexes)
                {
                    tasks.Add(ProcessSingleVoteAsync(llm, chunk, processed, voteIndex, ct));
                }
    ...
    private void NotifyCurrentMessage(int chunkIndex, int chunkCount)
    {
        ...
        int voteIndex, CancellationToken ct)
    {
        ...
        // int index = 0;
        var userPrompt = string.Join('\n', chunk.Select(x => $"【{x.Index}】{x.Input}"));
        //调用AI
        var response = await llm.CallAsync(GetSystemPrompt(chunk.Length), userPrompt, ct: ct);
        var lines = response.SplitLines();
        if (lines.Length != chunk.Length)
        {
            throw new AiUnexpectedFormatException($"AI返回的行数（{lines.Length}）与输入的行数（{chunk.Length}）不匹配。");
        }

        for (int i = 0; i < lines.Length; i++)
        {
            var line = indexPrefix.Replace(lines[i], string.Empty);
            if (voteIndex >= 0)
            {
                chunk[i].EachVote[voteIndex] = line;
            }
            else
            {
                chunk[i].Output = line;
